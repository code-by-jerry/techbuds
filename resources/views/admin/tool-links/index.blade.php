...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-2xl font-bold text-[#11224E]">Tool Links</h2>
            <p class="text-sm text-[#088395]/70 mt-1">Manage devtools links</p>
        </div>
        <a href="{{ route('admin.tool-links.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-[#088395] px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-[#37B7C3]">
    ...
    <div class="rounded-2xl border border-[#088395]/10 bg-white p-4 shadow-sm">
        <form method="GET" action="{{ route('admin.tool-links.index') }}" class="flex flex-col gap-4 sm:flex-row sm:items-end">
            <div class="flex-1 sm:max-w-xs">
                <label class="mb-1 block text-xs font-medium text-[#11224E]">Filter by Category</label>
                <select
                    name="category_id"
            ...
            <div>
                <a href="{{ route('admin.tool-links.index') }}" class="inline-block rounded-lg border border-[#088395]/20 px-4 py-2 text-sm font-medium text-[#088395] transition-colors hover:bg-[#088395]/5">
                    Clear Filter
                </a>
            </div>
            @endif
            ...
            <table class="w-full">
                <thead class="bg-[#088395]/5">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-[#11224E]">URL</th>
                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-[#11224E]">Order</th>
                ...
                <tbody class="divide-y divide-[#088395]/10 bg-white">
                    ...
                    <tr class="hover:bg-[#088395]/2 transition-colors">
                        <td class="px-6 py-4">
                            <div class="font-medium text-[#11224E]">{{ $link->title }}</div>
                            @if($link->description)
                                <div class="text-xs text-[#088395]/70 mt-1">{{ Str::limit($link->description, 60) }}</div>
                            @endif
                        </td>
                        <td class="px-6 py-4">
                            @if($link->category)
                        ...
                        <td class="px-6 py-4">
                            <a href="{{ $link->url }}" target="_blank" class="text-sm text-[#088395] hover:text-[#37B7C3] truncate max-w-xs block">
                        ...
                        <td class="px-6 py-4 text-right text-sm font-medium">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ $link->url }}" target="_blank" class="text-[#088395] hover:text-[#37B7C3]" title="View">
                                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                                    </svg>
                                </a>
                                <a href="{{ route('admin.tool-links.edit', $link) }}" class="text-[#088395] hover:text-[#37B7C3]" title="Edit">
                                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center">
                            <div class="mx-auto max-w-sm">
                                ...
                                <h3 class="mt-4 text-sm font-medium text-[#11224E]">No links found</h3>
                                <p class="mt-2 text-sm text-[#088395]/70">Get started by creating a new link.</p>
                                <div class="mt-6">
                                    <a href="{{ route('admin.tool-links.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-[#088395] px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-[#37B7C3]">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                                        </svg>
                                        Create New Link
        ...
        <div class="border-t border-[#088395]/10 px-6 py-4">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex items-center gap-2">
                    ...
                    @else
                    <a href="{{ $links->previousPageUrl() }}" class="rounded-lg border border-[#088395]/20 px-3 py-1.5 text-sm text-[#088395] transition-colors hover:bg-[#088395]/5">Previous</a>
                    @endif
                    ...
                    @for($page = $startPage; $page <= $endPage; $page++)
                        ...
                        @else
                        <a href="{{ $links->url($page) }}" class="rounded-lg border border-[#088395]/20 px-3 py-1.5 text-sm text-[#088395] transition-colors hover:bg-[#088395]/5">{{ $page }}</a>
                        @endif
                    @endfor

                    @if($links->hasMorePages())
                    <a href="{{ $links->nextPageUrl() }}" class="rounded-lg border border-[#088395]/20 px-3 py-1.5 text-sm text-[#088395] transition-colors hover:bg-[#088395]/5">Next</a>
                    @else
