...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-2xl font-bold text-heading">Contact Requests</h2>
            <p class="text-sm text-text-secondary mt-1">Manage contact enquiries and requests</p>
        </div>
    </div>
    ...
    <div class="rounded-2xl border border-border-default bg-surface-1 p-4 shadow-sm">
        <form method="GET" action="{{ route('admin.contacts.index') }}" class="flex flex-col gap-4 sm:flex-row sm:items-end">
            ...
            <!-- NDA Filter -->
            <div class="sm:w-48">
                <label class="mb-1 block text-xs font-medium text-heading">NDA Requested</label>
                <select
                    name="nda"
                    class="w-full rounded-lg border border-border-default bg-surface-2 px-3 py-2 text-sm text-heading focus:border-brand-primary focus:outline-none focus:ring-2 focus:ring-brand-primary/20"
                >
            ...
            <div>
                <a href="{{ route('admin.contacts.index') }}" class="inline-block rounded-lg border border-border-default px-4 py-2 text-sm font-medium text-brand-primary transition-colors hover:bg-brand-primary/5">
                    Clear
                </a>
            </div>
            @endif
            ...
            <table class="w-full">
                <thead class="bg-brand-primary/5">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-heading">Email</th>
                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-heading">Phone</th>
                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-heading">Project Type</th>
                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-heading">Status</th>
                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-heading">Date</th>
                ...
                <tbody class="divide-y divide-[var(--brand-primary)]/10 bg-surface-1">
                    ...
                    <tr class="hover:bg-brand-primary/2 transition-colors">
                        <td class="px-6 py-4">
                            ...
                            <span class="inline-flex items-center gap-1 mt-1 text-xs text-warning">
                        ...
                        <td class="px-6 py-4 text-sm text-text-secondary">
                            <a href="mailto:{{ $contact->email }}" class="hover:text-brand-primary transition-colors">
                                {{ $contact->email }}
                            </a>
                        </td>
                        <td class="px-6 py-4 text-sm text-text-secondary">
                        ...
                        <td class="px-6 py-4">
                            @php
                                $statusColors = [
                                    ...
                                    'in_progress' => 'bg-brand-soft/10 text-brand-soft',
                                    'completed' => 'bg-green-500/10 text-success',
                                ];
                                $color = $statusColors[$contact->status] ?? 'bg-surface-2 text-text-secondary';
                            @endphp
                        ...
                        <td class="px-6 py-4 text-right text-sm font-medium">
                            <a href="{{ route('admin.contacts.show', $contact) }}" class="text-brand-primary hover:text-brand-soft transition-colors" title="View">
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center">
                            <div class="mx-auto max-w-sm">
                                ...
                                <h3 class="mt-4 text-sm font-medium text-heading">No contact requests found</h3>
                                <p class="mt-2 text-sm text-text-secondary">No contact requests match your search criteria.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <div class="border-t border-border-default px-6 py-4">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex items-center gap-2">
                    ...
                    @else
                    <a href="{{ $contacts->appends(request()->query())->previousPageUrl() }}" class="rounded-lg border border-border-default px-3 py-1.5 text-sm text-brand-primary transition-colors hover:bg-brand-primary/5">Previous</a>
                    @endif
                    ...
                    @for($page = $startPage; $page <= $endPage; $page++)
                        ...
                        @else
                        <a href="{{ $contacts->appends(request()->query())->url($page) }}" class="rounded-lg border border-border-default px-3 py-1.5 text-sm text-brand-primary transition-colors hover:bg-brand-primary/5">{{ $page }}</a>
                        @endif
                    @endfor

                    @if($contacts->hasMorePages())
                    <a href="{{ $contacts->appends(request()->query())->nextPageUrl() }}" class="rounded-lg border border-border-default px-3 py-1.5 text-sm text-brand-primary transition-colors hover:bg-brand-primary/5">Next</a>
                    @else
