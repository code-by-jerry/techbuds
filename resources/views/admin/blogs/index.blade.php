...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-2xl font-bold text-heading">Blog Management</h2>
            <p class="text-sm text-text-secondary mt-1">Manage your SEO blog articles</p>
        </div>
        @php
        ...
        @endphp
        @if($canCreate)
        <a href="{{ route('admin.blogs.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-brand-primary px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-brand-hover">
    ...
    <div class="rounded-2xl border border-border-default bg-surface-1 p-4 shadow-sm">
        <form method="GET" action="{{ route('admin.blogs.index') }}" class="flex flex-col gap-4 sm:flex-row sm:items-end">
            ...
            <div class="sm:w-48">
                <label class="mb-1 block text-xs font-medium text-heading">Category</label>
                <select
                    name="category"
                    class="w-full rounded-lg border border-border-default bg-surface-2 px-3 py-2 text-sm text-text-primary placeholder:text-text-muted focus:border-brand-primary focus:outline-none focus:ring-2 focus:ring-brand-primary/20"
                >
            ...
            <div>
                <a href="{{ route('admin.blogs.index') }}" class="inline-block rounded-lg border border-border-default px-4 py-2 text-sm font-medium text-brand-primary transition-colors hover:bg-brand-primary/5">
                    Clear
                </a>
            </div>
            @endif
            ...
            <table class="w-full">
                <thead class="bg-brand-primary/5">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-heading">Published Date</th>
                    ...
                    <tr class="hover:bg-brand-primary/2 transition-colors">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                @else
                                <div class="flex h-12 w-12 items-center justify-center rounded-lg bg-gradient-to-br from-brand-primary to-brand-hover">
                                ...
                                @endif
                                <div>
                                    <div class="font-medium text-heading">{{ Str::limit($blog->title, 50) }}</div>
                                    <div class="text-xs text-text-secondary">{{ Str::limit($blog->excerpt, 60) }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                ...
                                @endif
                                @if($blog->is_featured)
                                <span class="inline-flex items-center gap-1 rounded-full bg-brand-soft/100/10 px-2.5 py-0.5 text-xs font-medium text-purple-500">
                                    <svg class="h-3 w-3" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <td class="px-6 py-4 text-right text-sm font-medium">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                @endphp
                                <a href="{{ route('blog.show', $blog->slug) }}" target="_blank" class="text-brand-primary hover:text-brand-soft" title="View">
                                ...
                                @if($canUpdate)
                                <a href="{{ route('admin.blogs.edit', $blog) }}" class="text-brand-primary hover:text-brand-soft" title="Edit">
                                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </a>
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center">
                            <div class="mx-auto max-w-sm">
                                ...
                                <h3 class="mt-4 text-sm font-medium text-heading">No blogs found</h3>
                                <p class="mt-2 text-sm text-text-secondary">Get started by creating a new blog post.</p>
                                <div class="mt-6">
                                    <a href="{{ route('admin.blogs.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-brand-primary px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-brand-hover">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                                        </svg>
                                        Create New Blog
        ...
        <div class="border-t border-border-default px-6 py-4">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex items-center gap-2">
                    ...
                    @else
                    <a href="{{ $blogs->appends(request()->query())->previousPageUrl() }}" class="rounded-lg border border-border-default px-3 py-1.5 text-sm text-brand-primary transition-colors hover:bg-brand-primary/5">Previous</a>
                    @endif
                    ...
                    @for($page = $startPage; $page <= $endPage; $page++)
                        ...
                        @else
                        <a href="{{ $blogs->appends(request()->query())->url($page) }}" class="rounded-lg border border-border-default px-3 py-1.5 text-sm text-brand-primary transition-colors hover:bg-brand-primary/5">{{ $page }}</a>
                        @endif
                    @endfor

                    @if($blogs->hasMorePages())
                    <a href="{{ $blogs->appends(request()->query())->nextPageUrl() }}" class="rounded-lg border border-border-default px-3 py-1.5 text-sm text-brand-primary transition-colors hover:bg-brand-primary/5">Next</a>
                    @else
