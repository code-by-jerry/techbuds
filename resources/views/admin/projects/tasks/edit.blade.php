...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <div class="flex items-center gap-3">
                ...
                <h2 class="text-2xl font-bold text-heading">Edit Task</h2>
            </div>
            <p class="text-sm text-brand-primary/70 mt-1">Update task for {{ $project->title }}</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.tasks.update', $task) }}" method="POST" class="space-y-6">
        ...
        <div class="rounded-2xl border border-border-default bg-surface-1 p-6 shadow-sm">
            <h3 class="mb-4 text-lg font-semibold text-heading">Task Information</h3>
            <div class="space-y-4">
                <!-- Title -->
                <div>
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            class="w-full rounded-lg border border-[var(--brand-primary)]/15 px-3 py-2 text-sm text-heading focus:border-[var(--brand-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--brand-primary)]/20"
                        >
                            <option value="0" {{ old('priority', $task->priority) == '0' ? 'selected' : '' }}>None</option>
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <div>
                        <label for="milestone_id" class="mb-1 block text-sm font-medium text-heading">Milestone</label>
                        <select
                            id="milestone_id"
                            name="milestone_id"
                            class="w-full rounded-lg border border-[var(--brand-primary)]/15 px-3 py-2 text-sm text-heading focus:border-[var(--brand-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--brand-primary)]/20"
                ...
                <!-- Internal Comments -->
                <div>
                    <label for="internal_comments" class="mb-1 block text-sm font-medium text-heading">Internal Comments</label>
                    <textarea
                        id="internal_comments"
                        name="internal_comments"
                        rows="3"
                        class="w-full rounded-lg border border-[var(--brand-primary)]/15 px-3 py-2 text-sm text-heading focus:border-[var(--brand-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--brand-primary)]/20"
                    >{{ old('internal_comments', $task->internal_comments) }}</textarea>
