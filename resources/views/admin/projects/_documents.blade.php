<div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
        <h4 class="text-lg font-semibold text-[#11224E]">Documents</h4>
        <a href="{{ route('admin.projects.documents.create', $project) }}" class="inline-flex items-center gap-2 rounded-lg bg-[#088395] px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-[#37B7C3]">
            ...
            <div class="flex items-start justify-between">
                <div class="flex-1">
                    <div class="flex items-center gap-3">
                        ...
                        <h5 class="font-medium text-[#11224E]">{{ $document->name }}</h5>
                        @php
                            $categoryColors = [
                                'nda' => 'bg-blue-50 text-blue-600',
                                'proposal' => 'bg-green-50 text-green-600',
                                'quote' => 'bg-yellow-50 text-yellow-600',
                                'invoice' => 'bg-orange-50 text-orange-600',
                                'contract' => 'bg-purple-50 text-purple-600',
                                'final_delivery' => 'bg-indigo-50 text-indigo-600',
                                'offboarding' => 'bg-gray-50 text-gray-600',
                                'other' => 'bg-gray-50 text-gray-600',
                            ];
                            $categoryColor = $categoryColors[$document->category] ?? 'bg-gray-50 text-gray-600';
                        @endphp
                    ...
                    <p class="text-sm text-[#088395]/70 mt-2">{{ Str::limit($document->description, 150) }}</p>
                    @endif
                ...
                <div class="flex items-center gap-2 ml-4">
                    <a href="{{ route('admin.documents.download', $document) }}" class="text-[#088395] hover:text-[#37B7C3]" title="Download">
                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                        </svg>
                    </a>
                    <a href="{{ route('admin.documents.edit', $document) }}" class="text-[#088395] hover:text-[#37B7C3]" title="Edit">
                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="rounded-lg border border-[#088395]/10 bg-white p-8 text-center">
        ...
        <h3 class="mt-4 text-sm font-medium text-[#11224E]">No documents yet</h3>
        <p class="mt-2 text-sm text-[#088395]/70">Start uploading project documents.</p>
        <div class="mt-6">
            <a href="{{ route('admin.projects.documents.create', $project) }}" class="inline-flex items-center gap-2 rounded-lg bg-[#088395] px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-[#37B7C3]">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
